...
const register = async (req, res) => {
    ...
    //     to: user.email,
    //     subject: 'Erm... hi!',
    //     html: '<p>Contact about extended warranty</p>',
    // });
...
const verifyEmail = async (req, res) => {
    ...
    if (!user) {
        throw new CustomError.UnauthenticatedError('Validation failed (no user)');
    }

    if (verificationToken !== user.verificationToken) {
        throw new CustomError.UnauthenticatedError('Validation failed (no token)');
    }
...
const forgotPassword = async (req, res) => {
    ...
    res.status(StatusCodes.OK).json({ msg: 'Please check email for reset password link' });
};
